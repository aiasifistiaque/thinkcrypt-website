	...
	return (
		...
		>
			...
			<Column display={{ base: 'flex', md: 'none' }}>
				...
				<Column ml={6}>
					<Accordion defaultIndex={[0]} borderLeft={BORDER}>
						{howWeWork?.map((item, index) => (
							<AccordionItem key={index} border={BORDER}>
								...
								<AccordionPanel pb={4}>
									<Text fontSize='14px' fontFamily='Suisse'>
										{item.subTitle}
									</Text>
								</AccordionPanel>
							</AccordionItem>
			...
			>
				<Grid templateColumns='2fr 3fr' w='full'>
					...
					<Grid templateRows='1fr 1fr' w='full'>
						<Flex w='full' p={8}>
							<Text fontSize='48px' fontFamily={fonts.heading} fontWeight='600'>
								Our Services
							</Text>
						</Flex>
						<CustomImage w='full' h='full'>
				...
				>
					{howWeWork?.map((item, index) => (
						...
						>
							<Text fontSize='24px' fontWeight='400' fontFamily={fonts.heading}>
								{item.title}
							</Text>
							<Text
								fontFamily='Suisse'
