...
import { Box, Center, Flex, Grid, GridItem, Switch, Text } from '@chakra-ui/react';
...
const PlanSection = () => {
	...
	const advancedFeatures = planData.find(plan => plan.item === 'Advanced')['data-list'];
		...
		<Column
			...
			<Flex
				...
				<Flex
					...
					<P fontFamily={fonts.Michroma}>Pay Monthly</P>
					<Switch
					...
					/>
					<P fontFamily={fonts.Michroma}>Pay Annually</P>
				</Flex>
				<P
					...
					<GridItem key={plan?.id}>
						<Box
							...
							<Text
								...
								mb={4}
								fontFamily={fonts.Michroma}>
								Save BDT {isYearly ? plan['yearly-savings'] : plan['monthly-savings']}
							...
							<Column
								...
								{advancedFeatures
									...
									.sort((a, b) => {
										...
										return isAIncluded === isBIncluded ? 0 : isAIncluded ? -1 : 1;
									})
									.map((feature, index) => (
										<Flex
											key={index}
